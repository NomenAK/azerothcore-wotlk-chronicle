...
## Git & PR workflow (Capy)

These rules keep PRs linked to their Capy task/thread. A PR opened with a raw
`gh pr create` on a non-`capy/*` branch is **not** attached to the thread. The
default branch of this fork is `Playerbot`.

1. **Always work on a `capy/<slug>` branch.** Step 1 of any task:
   `git fetch origin Playerbot && git checkout -B capy/<slug> origin/Playerbot`.
...
3. Before pushing a parallel branch: `git fetch origin Playerbot && git rebase origin/Playerbot`,
